    ...
    while (i < weight) {
        do {
            if (j == random_bytes_size) {
                hqcv5_192_xof_get_bytes(ctx, rand_bytes, random_bytes_size);
                j = 0;
            }
...
void hqcv5_192_vect_generate_random_support2(hqcv5_192_shake256_xof_ctx *ctx, uint32_t *support, uint16_t weight) {
    ...
    hqcv5_192_xof_get_bytes(ctx, (uint8_t *)&rand_u32, 4 * weight);
...
void hqcv5_192_vect_set_random(hqcv5_192_shake256_xof_ctx *ctx, uint64_t *v) {
    hqcv5_192_xof_get_bytes(ctx, (uint8_t *)v, hqcv5_192_VEC_N_SIZE_BYTES);
