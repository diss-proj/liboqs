...
/**
 * @brief Constant-time Barrett reduction modulo hqcv5_192_PARAM_N.
 *
 * Reduces \p x modulo hqcv5_192_PARAM_N using the precomputed value hqcv5_192_PARAM_N_MU = ⌊2^32 / hqcv5_192_PARAM_N⌋.
 *
 * @param[in] x Input value to reduce.
 * @return x mod hqcv5_192_PARAM_N in constant time.
 */
static inline uint32_t barrett_reduce(uint32_t x) {
    uint64_t q = ((uint64_t)x * hqcv5_192_PARAM_N_MU) >> 32;
    uint32_t r = x - (uint32_t)(q * hqcv5_192_PARAM_N);

    uint32_t reduce_flag = (((r - hqcv5_192_PARAM_N) >> 31) ^ 1);
    uint32_t mask = -reduce_flag;
    r -= mask & hqcv5_192_PARAM_N;
    return r;
}
 ...
 * distinct indices uniformly at random from the interval [0, hqcv5_192_PARAM_N).
 * It ensures that the output is non-biased and the values are uniformly distributed.
 *
 * Internally, it samples 24-bit random values and rejects any value ≥ hqcv5_192_UTILS_REJECTION_THRESHOLD,
 ...
 */
void hqcv5_192_vect_generate_random_support1(shake256_xof_ctx *ctx, uint32_t *support, uint16_t weight) {
    size_t random_bytes_size = 3 * weight;
    uint8_t rand_bytes[3 * hqcv5_192_PARAM_OMEGA_R] = {0};
    uint8_t inc;
    size_t i, j;
    ...
    while (i < weight) {
        ...
        } while (support[i] >= hqcv5_192_UTILS_REJECTION_THRESHOLD);
 ...
 */
void hqcv5_192_vect_generate_random_support2(shake256_xof_ctx *ctx, uint32_t *support, uint16_t weight) {
    uint32_t rand_u32[hqcv5_192_PARAM_OMEGA_R] = {0};

    xof_get_bytes(ctx, (uint8_t *)&rand_u32, 4 * weight);

    for (size_t i = 0; i < weight; ++i) {
        uint64_t buff = rand_u32[i];
        support[i] = i + ((buff * (hqcv5_192_PARAM_N - i)) >> 32);
    }
...
void hqcv5_192_vect_write_support_to_vector(uint64_t *v, uint32_t *support, uint16_t weight) {
    uint32_t index_tab[hqcv5_192_PARAM_OMEGA_R] = {0};
    uint64_t bit_tab[hqcv5_192_PARAM_OMEGA_R] = {0};
    ...
    uint64_t val = 0;
    for (uint32_t i = 0; i < hqcv5_192_VEC_N_SIZE_64; i++) {
        val = 0;
 ...
 * @param[out]    v       Pointer to an array of ⌈hqcv5_192_PARAM_N/64⌉ 64-bit words.
 *                        On return, **v** is a bitmask with exactly `weight`
 *                        bits set to 1.
 * @param[in]     weight  Desired Hamming weight.
 */
void hqcv5_192_vect_sample_fixed_weight1(shake256_xof_ctx *ctx, uint64_t *v, uint16_t weight) {
    uint32_t support[hqcv5_192_PARAM_OMEGA_R] = {0};
    hqcv5_192_vect_generate_random_support1(ctx, support, weight);
    hqcv5_192_vect_write_support_to_vector(v, support, weight);
}
 ...
 * @param[in]     weight  Desired Hamming weight.
 */
void hqcv5_192_vect_sample_fixed_weight2(shake256_xof_ctx *ctx, uint64_t *v, uint16_t weight) {
    uint32_t support[hqcv5_192_PARAM_OMEGA_R] = {0};
    hqcv5_192_vect_generate_random_support2(ctx, support, weight);
    hqcv5_192_vect_write_support_to_vector(v, support, weight);
}

/**
 * @brief Generates a random vector of dimension <b>hqcv5_192_PARAM_N</b>
 *
 * This function generates a random binary vector of dimension <b>hqcv5_192_PARAM_N</b>. It generates a random
 * array of bytes using the xof, and drop the extra bits using a mask.
 *
 * @param[in] ctx Pointer to the context of the xof
 * @param[in] v Pointer to an array
 */
void hqcv5_192_vect_set_random(shake256_xof_ctx *ctx, uint64_t *v) {
    xof_get_bytes(ctx, (uint8_t *)v, hqcv5_192_VEC_N_SIZE_BYTES);
    v[hqcv5_192_VEC_N_SIZE_64 - 1] &= BITMASK(hqcv5_192_PARAM_N, 64);
}
...
/**
 * Truncate the bit-array v in-place to hqcv5_192_PARAM_N1N2 bits,
...
void hqcv5_192_vect_truncate(uint64_t *v) {
    size_t orig_words = (hqcv5_192_PARAM_N + 63) / 64;
    size_t new_full_words = hqcv5_192_PARAM_N1N2 / 64;
    size_t remaining_bits = hqcv5_192_PARAM_N1N2 % 64;
...
void hqcv5_192_vect_print(const uint64_t *v, const uint32_t size) {
    if (size == hqcv5_192_VEC_K_SIZE_BYTES) {
        uint8_t tmp[hqcv5_192_VEC_K_SIZE_BYTES] = {0};
        memcpy(tmp, v, hqcv5_192_VEC_K_SIZE_BYTES);
        for (uint32_t i = 0; i < hqcv5_192_VEC_K_SIZE_BYTES; ++i) {
            printf("%02x", tmp[i]);
        }
    } else if (size == hqcv5_192_VEC_N_SIZE_BYTES) {
        uint8_t tmp[hqcv5_192_VEC_N_SIZE_BYTES] = {0};
        memcpy(tmp, v, hqcv5_192_VEC_N_SIZE_BYTES);
        for (uint32_t i = 0; i < hqcv5_192_VEC_N_SIZE_BYTES; ++i) {
            printf("%02x", tmp[i]);
        }
    } else if (size == hqcv5_192_VEC_N1N2_SIZE_BYTES) {
        uint8_t tmp[hqcv5_192_VEC_N1N2_SIZE_BYTES] = {0};
        memcpy(tmp, v, hqcv5_192_VEC_N1N2_SIZE_BYTES);
        for (uint32_t i = 0; i < hqcv5_192_VEC_N1N2_SIZE_BYTES; ++i) {
            printf("%02x", tmp[i]);
        }
    } else if (size == hqcv5_192_VEC_N1_SIZE_BYTES) {
        uint8_t tmp[hqcv5_192_VEC_N1_SIZE_BYTES] = {0};
        memcpy(tmp, v, hqcv5_192_VEC_N1_SIZE_BYTES);
        for (uint32_t i = 0; i < hqcv5_192_VEC_N1_SIZE_BYTES; ++i) {
            printf("%02x", tmp[i]);
        }
    }
}
