 ...
 *  - At each recursion level on words of length n, hqcv5_192_karatsuba_mul needs 8*n words.
 ...
 *  - We set hqcv5_192_TMP_BUFFER_WORDS = 16 * hqcv5_192_VEC_N_SIZE_64 to guarantee enough space for all recursion levels.
 */
...
/** @def hqcv5_192_KARATSUBA_THRESHOLD
 *  @brief Input size (in words) below which hqcv5_192_schoolbook_mul is used.
 */
#define hqcv5_192_KARATSUBA_THRESHOLD 16

/** @def hqcv5_192_TMP_BUFFER_WORDS
 *  @brief Total size in 64-bit words for the temporary buffer used by recursive Karatsuba.
 */
#define hqcv5_192_TMP_BUFFER_WORDS (16 * hqcv5_192_VEC_N_SIZE_64)
 ...
 */
static void hqcv5_192_schoolbook_mul(uint64_t *r, const uint64_t *a, const uint64_t *b, size_t n) {
 ...
 *
 * If n <= hqcv5_192_KARATSUBA_THRESHOLD, falls back to hqcv5_192_schoolbook_mul.
 ...
 */
static void hqcv5_192_karatsuba_mul(uint64_t *r, const uint64_t *a, const uint64_t *b, size_t n, uint64_t *tmp_buffer) {
    if (n <= hqcv5_192_KARATSUBA_THRESHOLD) {
        hqcv5_192_schoolbook_mul(r, a, b, n);
        return;
    }
    ...
    /* 1) low * low */
    hqcv5_192_karatsuba_mul(z0, a, b, n0, child_buffer);

    /* 2) high * high */
    hqcv5_192_karatsuba_mul(z2, a + m, b + m, n1, child_buffer);
    ...
    hqcv5_192_karatsuba_mul(zmid, ta, tb, n1, child_buffer);
 ...
 *
 * @param[out] o  Result buffer, size hqcv5_192_VEC_N_SIZE_64 words.
 * @param[in]  a  Input buffer, size 2*hqcv5_192_VEC_N_SIZE_64 words.
 */
static void hqcv5_192_reduce(uint64_t *o, const uint64_t *a) {
    for (size_t i = 0; i < hqcv5_192_VEC_N_SIZE_64; i++) {
        uint64_t r = a[i + hqcv5_192_VEC_N_SIZE_64 - 1] >> (hqcv5_192_PARAM_N & 0x3F);
        uint64_t carry = a[i + hqcv5_192_VEC_N_SIZE_64] << (64 - (hqcv5_192_PARAM_N & 0x3F));
        o[i] = a[i] ^ r ^ carry;
    }
    o[hqcv5_192_VEC_N_SIZE_64 - 1] &= BITMASK(hqcv5_192_PARAM_N, 64);
}

/**
 * @brief Carry-less multiplication mod (X^hqcv5_192_PARAM_N - 1).
 *
 * Computes o = a1 * a2, each operand of hqcv5_192_VEC_N_SIZE_64 words, then reduces.
 *
 * @param[out] o   Result buffer, size hqcv5_192_VEC_N_SIZE_64 words.
 * @param[in]  a1  Operand polynomial a(x).
 * @param[in]  a2  Operand polynomial b(x).
 */
void hqcv5_192_vect_mul(uint64_t *o, const uint64_t *a1, const uint64_t *a2) {
    uint64_t unreduced[2 * hqcv5_192_VEC_N_SIZE_64];
    uint64_t tmp_buffer[hqcv5_192_TMP_BUFFER_WORDS];

    /* multiply via Karatsuba into unreduced */
    hqcv5_192_karatsuba_mul(unreduced, a1, a2, hqcv5_192_VEC_N_SIZE_64, tmp_buffer);

    /* reduce modulo X^n - 1 */
    hqcv5_192_reduce(o, unreduced);
}
