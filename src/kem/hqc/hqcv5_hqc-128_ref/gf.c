...
static uint16_t hqcv5_128_gf_reduce(uint16_t x);
void hqcv5_128_gf_carryless_mul(uint8_t *c, uint8_t a, uint8_t b);
 ...
 * The last two elements of the exp table are needed by the hqcv5_128_gf_mul function from gf_lutmul.c
...
void hqcv5_128_gf_generate(uint16_t *exp, uint16_t *log, const int16_t m) {
    ...
    uint16_t gf_poly = hqcv5_128_PARAM_GF_POLY;
...
/**
 * @brief Feedback bit positions used for modular reduction by hqcv5_128_PARAM_GF_POLY = 0x11D.
 *
 * These values are derived from the binary form of the polynomial:
 *     0x11D = 0b100011101 → bits set at positions: 8, 4, 3, 1, 0
 *
 * To hqcv5_128_reduce a polynomial modulo this irreducible polynomial:
 ...
 */
static const uint8_t hqcv5_128_gf_reduction_taps[] = {4, 3, 2};

/**
 * @brief Reduce a polynomial modulo hqcv5_128_PARAM_GF_POLY in GF(2^8).
 *
 * This function performs modular reduction of a 16-bit polynomial `x`
 * by the irreducible polynomial hqcv5_128_PARAM_GF_POLY = 0x11D
 ...
 *
 * @param x 16-bit input polynomial to hqcv5_128_reduce (deg(x) ≤ 14)
 * @return Reduced 8-bit polynomial modulo hqcv5_128_PARAM_GF_POLY (deg(x) < 8)
 */
uint16_t hqcv5_128_gf_reduce(uint16_t x) {
    uint64_t mod;
    const int reduction_steps = 2;            // For deg(x) = 2 * (8 - 1) = 14, hqcv5_128_reduce twice to bring degree < 8
    const size_t gf_reduction_tap_count = 3;  // Number of feedback positions

    for (int i = 0; i < reduction_steps; ++i) {
        mod = x >> hqcv5_128_PARAM_M;       // Extract upper bits
        x &= (1 << hqcv5_128_PARAM_M) - 1;  // Keep lower bits
        x ^= mod;                 // Pre-XOR with no shift

        uint16_t z1 = 0;
        for (size_t j = gf_reduction_tap_count; j; --j) {
            uint16_t z2 = hqcv5_128_gf_reduction_taps[j - 1];
            uint16_t dist = z2 - z1;
            mod <<= dist;
            x ^= mod;
            z1 = z2;
...
uint16_t hqcv5_128_gf_mul(uint16_t a, uint16_t b) {
    uint8_t c[2] = {0};
    hqcv5_128_gf_carryless_mul(c, (uint8_t)a, (uint8_t)b);
    uint16_t tmp = (uint16_t)(c[0] ^ (c[1] << 8));
    return hqcv5_128_gf_reduce(tmp);
}
...
uint16_t hqcv5_128_gf_square(uint16_t a) {
    uint32_t b = a;
    uint32_t s = b & 1;
    for (size_t i = 1; i < hqcv5_128_PARAM_M; ++i) {
        b <<= 1;
        s ^= b & (1 << 2 * i);
    }

    return hqcv5_128_gf_reduce(s);
}
...
uint16_t hqcv5_128_gf_inverse(uint16_t a) {
    ...
    inv = hqcv5_128_gf_square(a);       /* a^2 */
    tmp1 = hqcv5_128_gf_mul(inv, a);    /* a^3 */
    inv = hqcv5_128_gf_square(inv);     /* a^4 */
    tmp2 = hqcv5_128_gf_mul(inv, tmp1); /* a^7 */
    tmp1 = hqcv5_128_gf_mul(inv, tmp2); /* a^11 */
    inv = hqcv5_128_gf_mul(tmp1, inv);  /* a^15 */
    inv = hqcv5_128_gf_square(inv);     /* a^30 */
    inv = hqcv5_128_gf_square(inv);     /* a^60 */
    inv = hqcv5_128_gf_square(inv);     /* a^120 */
    inv = hqcv5_128_gf_mul(inv, tmp2);  /* a^127 */
    inv = hqcv5_128_gf_square(inv);     /* a^254 */
    return inv;
}
