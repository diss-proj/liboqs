 ...
 *
 * Calculates the ceiling of hqcv5_128_PARAM_N2/128 to determine how many
 * copies of each 128-bit codeword are used in the code expansion.
 */
#define hqcv5_128_MULTIPLICITY CEIL_DIVIDE(hqcv5_128_PARAM_N2, 128)
 ...
 */
#define hqcv5_128_BIT0MASK(x) (int32_t)(-((x) & 1))
// clang-format on

void hqcv5_128_encode(hqcv5_128_rm_codeword_t *word, int32_t message);
void hqcv5_128_hadamard(rm_expanded_cdw *src, rm_expanded_cdw *dst);
void hqcv5_128_expand_and_sum(rm_expanded_cdw *dest, hqcv5_128_rm_codeword_t src[]);
int32_t hqcv5_128_find_peaks(rm_expanded_cdw *transform);
...
void hqcv5_128_encode(hqcv5_128_rm_codeword_t *word, int32_t message) {
    ...
    first_word = hqcv5_128_BIT0MASK(message >> 7);

    first_word ^= hqcv5_128_BIT0MASK(message >> 0) & 0xaaaaaaaa;
    first_word ^= hqcv5_128_BIT0MASK(message >> 1) & 0xcccccccc;
    first_word ^= hqcv5_128_BIT0MASK(message >> 2) & 0xf0f0f0f0;
    first_word ^= hqcv5_128_BIT0MASK(message >> 3) & 0xff00ff00;
    first_word ^= hqcv5_128_BIT0MASK(message >> 4) & 0xffff0000;

    word->u32[0] = first_word;

    first_word ^= hqcv5_128_BIT0MASK(message >> 5);
    word->u32[1] = first_word;
    first_word ^= hqcv5_128_BIT0MASK(message >> 6);
...
void hqcv5_128_expand_and_sum(rm_expanded_cdw *dest, hqcv5_128_rm_codeword_t src[]) {
    ...
    for (int32_t copy = 1; copy < hqcv5_128_MULTIPLICITY; copy++) {
        for (int32_t part = 0; part < 4; part++) {
            for (int32_t bit = 0; bit < 32; bit++) {
 ...
 *
 * The message consists of N1 bytes each byte is encoded into hqcv5_128_PARAM_N2 bits,
 * or hqcv5_128_MULTIPLICITY repeats of 128 bits
 *
 * @param[out] cdw Array of size hqcv5_128_VEC_N1N2_SIZE_64 receiving the encoded message
 * @param[in] msg Array of size hqcv5_128_VEC_N1_SIZE_64 storing the message
 */
void hqcv5_128_reed_muller_encode(uint64_t *cdw, const uint64_t *msg) {
    uint8_t *message_array = (uint8_t *)msg;
    hqcv5_128_rm_codeword_t *codeArray = (hqcv5_128_rm_codeword_t *)cdw;
    for (size_t i = 0; i < hqcv5_128_VEC_N1_SIZE_BYTES; i++) {
        // fill entries i * hqcv5_128_MULTIPLICITY to (i+1) * hqcv5_128_MULTIPLICITY
        int32_t pos = i * hqcv5_128_MULTIPLICITY;
        // encode first word
        hqcv5_128_encode(&codeArray[pos], message_array[i]);
        // copy to other identical codewords
        for (size_t copy = 1; copy < hqcv5_128_MULTIPLICITY; copy++) {
            memcpy(&codeArray[pos + copy], &codeArray[pos], sizeof(hqcv5_128_rm_codeword_t));
        }
    }
}
 ...
 *
 * @param[out] msg Array of size hqcv5_128_VEC_N1_SIZE_64 receiving the decoded message
 * @param[in] cdw Array of size hqcv5_128_VEC_N1N2_SIZE_64 storing the received word
 */
void hqcv5_128_reed_muller_decode(uint64_t *msg, const uint64_t *cdw) {
    uint8_t *message_array = (uint8_t *)msg;
    hqcv5_128_rm_codeword_t *codeArray = (hqcv5_128_rm_codeword_t *)cdw;
    rm_expanded_cdw expanded;
    for (size_t i = 0; i < hqcv5_128_VEC_N1_SIZE_BYTES; i++) {
        // collect the codewords
        hqcv5_128_expand_and_sum(&expanded, &codeArray[i * hqcv5_128_MULTIPLICITY]);
        // apply hadamard transform
        rm_expanded_cdw transform;
        hqcv5_128_hadamard(&expanded, &transform);
        // fix the first entry to get the half Hadamard transform
        transform[0] -= 64 * hqcv5_128_MULTIPLICITY;
        // finish the decoding
        message_array[i] = hqcv5_128_find_peaks(&transform);
    }
}
