...
/**
 * @typedef hqcv5_256_shake256_prng_ctx
 * @brief Incremental SHAKE-256 prng context.
 *
 */
shake256incctx hqcv5_256_shake256_prng_ctx;
 ...
 */
void hqcv5_256_prng_init(uint8_t *entropy_input, uint8_t *personalization_string, uint32_t enlen, uint32_t perlen) {
    uint8_t domain = hqcv5_256_HQC_PRNG_DOMAIN;
    shake256_inc_init(&hqcv5_256_shake256_prng_ctx);
    shake256_inc_absorb(&hqcv5_256_shake256_prng_ctx, entropy_input, enlen);
    shake256_inc_absorb(&hqcv5_256_shake256_prng_ctx, personalization_string, perlen);
    shake256_inc_absorb(&hqcv5_256_shake256_prng_ctx, &domain, 1);
    shake256_inc_finalize(&hqcv5_256_shake256_prng_ctx);
}
 ...
 */
void hqcv5_256_prng_get_bytes(uint8_t *output, uint32_t outlen) {
    shake256_inc_squeeze(output, outlen, &hqcv5_256_shake256_prng_ctx);
}
 ...
 */
void hqcv5_256_xof_init(hqcv5_256_shake256_xof_ctx *xof_ctx, const uint8_t *seed, uint32_t seed_size) {
    uint8_t xof_domain = hqcv5_256_HQC_XOF_DOMAIN;
 ...
 * The context must have been initialized beforehand using `hqcv5_256_xof_init()`.
 */
void hqcv5_256_xof_get_bytes(hqcv5_256_shake256_xof_ctx *xof_ctx, uint8_t *output, uint32_t output_size) {
 ...
 */
void hqcv5_256_hash_i(uint8_t *output, const uint8_t *seed) {
    hqcv5_256_sha3_512_ctx i_hash_ctx = {0};
    uint8_t i_domain = hqcv5_256_HQC_I_FCT_DOMAIN;
 ...
 */
void hqcv5_256_hash_h(uint8_t *output, const uint8_t ek_kem[hqcv5_256_PUBLIC_KEY_BYTES]) {
    hqcv5_256_sha3_256_ctx h_hash_ctx = {0};
    uint8_t h_domain = hqcv5_256_HQC_H_FCT_DOMAIN;
 ...
 */
void hqcv5_256_hash_g(uint8_t *output, const uint8_t hash_ek_kem[hqcv5_256_SEED_BYTES], const uint8_t m[hqcv5_256_PARAM_SECURITY_BYTES],
            const uint8_t salt[hqcv5_256_SALT_BYTES]) {
    hqcv5_256_sha3_512_ctx g_hash_ctx = {0};
    uint8_t i_domain = hqcv5_256_HQC_G_FCT_DOMAIN;
 ...
 */
void hqcv5_256_hash_j(uint8_t *output, const uint8_t hash_ek_kem[hqcv5_256_SEED_BYTES], const uint8_t sigma[hqcv5_256_PARAM_SECURITY_BYTES],
            const hqcv5_256_ciphertext_kem_t *c_kem) {
    hqcv5_256_sha3_256_ctx k_hash_ctx = {0};
    uint8_t k_domain = hqcv5_256_HQC_J_FCT_DOMAIN;
