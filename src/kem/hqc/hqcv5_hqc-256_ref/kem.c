...
int hqcv5_256_crypto_kem_keypair(uint8_t *ek_kem, uint8_t *dk_kem) {
    ...
    memcpy(ek_kem, ek_pke, hqcv5_256_PUBLIC_KEY_BYTES);
    memcpy(dk_kem, ek_kem, hqcv5_256_PUBLIC_KEY_BYTES);
    memcpy(dk_kem + hqcv5_256_PUBLIC_KEY_BYTES, dk_pke, hqcv5_256_SEED_BYTES);
    memcpy(dk_kem + hqcv5_256_PUBLIC_KEY_BYTES + hqcv5_256_SEED_BYTES, sigma, hqcv5_256_PARAM_SECURITY_BYTES);
    memcpy(dk_kem + hqcv5_256_PUBLIC_KEY_BYTES + hqcv5_256_SEED_BYTES + hqcv5_256_PARAM_SECURITY_BYTES, seed_kem, hqcv5_256_SEED_BYTES);

#ifdef VERBOSE
    printf("\n\nseed_kem: ");
    for (int i = 0; i < hqcv5_256_SEED_BYTES; ++i) printf("%02x", seed_kem[i]);
    printf("\n\nseed_pke: ");
    for (int i = 0; i < hqcv5_256_SEED_BYTES; ++i) printf("%02x", seed_pke[i]);
    printf("\n\nsigma: ");
    for (int i = 0; i < hqcv5_256_PARAM_SECURITY_BYTES; ++i) printf("%02x", sigma[i]);
#endif

    // Zeroize sensitive data
    hqcv5_256_memset_zero(seed_kem, sizeof seed_kem);
    hqcv5_256_memset_zero(sigma, sizeof sigma);
    hqcv5_256_memset_zero(seed_pke, sizeof seed_pke);
    hqcv5_256_memset_zero(dk_pke, sizeof dk_pke);
...
int hqcv5_256_crypto_kem_enc(uint8_t *c_kem, uint8_t *K, const uint8_t *ek_kem) {
    ...
    uint8_t m[hqcv5_256_PARAM_SECURITY_BYTES] = {0};
    uint8_t K_theta[hqcv5_256_SHARED_SECRET_BYTES + hqcv5_256_SEED_BYTES] = {0};
    uint8_t theta[hqcv5_256_SEED_BYTES] = {0};
    uint8_t hash_ek_kem[hqcv5_256_SEED_BYTES] = {0};
    ...
    hqcv5_256_hqc_c_kem_to_string(c_kem, &c_kem_t);
    memcpy(K, K_theta, hqcv5_256_SHARED_SECRET_BYTES);

#ifdef VERBOSE
    printf("\n\nek_kem: ");
    for (int i = 0; i < hqcv5_256_PUBLIC_KEY_BYTES; ++i) printf("%02x", ek_kem[i]);
    printf("\n\nm: ");
    hqcv5_256_vect_print((uint64_t *)m, hqcv5_256_PARAM_SECURITY_BYTES);
    printf("\n\nsalt: ");
    for (int i = 0; i < hqcv5_256_SALT_BYTES; ++i) printf("%02x", c_kem_t.salt[i]);
    printf("\n\nH(ek_kem): ");
    for (int i = 0; i < hqcv5_256_SEED_BYTES; ++i) printf("%02x", hash_ek_kem[i]);
    printf("\n\ntheta: ");
    for (int i = 0; i < hqcv5_256_SEED_BYTES; ++i) printf("%02x", theta[i]);
    printf("\n\nc_kem: ");
    for (int i = 0; i < hqcv5_256_CIPHERTEXT_BYTES; ++i) printf("%02x", c_kem[i]);
    printf("\n\nK: ");
    for (int i = 0; i < hqcv5_256_SHARED_SECRET_BYTES; ++i) printf("%02x", K[i]);
#endif

    // Zeroize sensitive data
    hqcv5_256_memset_zero(m, sizeof m);
    hqcv5_256_memset_zero(K_theta, sizeof K_theta);
    hqcv5_256_memset_zero(theta, sizeof theta);
...
int hqcv5_256_crypto_kem_dec(uint8_t *K_prime, const uint8_t *c_kem, const uint8_t *dk_kem) {
    ...
    uint8_t dk_pke[hqcv5_256_SEED_BYTES] = {0};
    uint8_t sigma[hqcv5_256_PARAM_SECURITY_BYTES] = {0};
    uint8_t m_prime[hqcv5_256_PARAM_SECURITY_BYTES] = {0};
    uint8_t hash_ek_kem[hqcv5_256_SEED_BYTES] = {0};
    uint8_t K_theta_prime[hqcv5_256_SHARED_SECRET_BYTES + hqcv5_256_SEED_BYTES] = {0};
    uint8_t K_bar[hqcv5_256_SHARED_SECRET_BYTES] = {0};
    uint8_t theta_prime[hqcv5_256_SEED_BYTES] = {0};
    hqcv5_256_ciphertext_kem_t c_kem_t = {0};
    hqcv5_256_ciphertext_kem_t c_kem_prime_t = {0};
    uint8_t result;

    // Parse decapsulation key dk_kem
    memcpy(ek_pke, dk_kem, hqcv5_256_PUBLIC_KEY_BYTES);
    memcpy(dk_pke, dk_kem + hqcv5_256_PUBLIC_KEY_BYTES, hqcv5_256_SEED_BYTES);
    memcpy(sigma, dk_kem + hqcv5_256_PUBLIC_KEY_BYTES + hqcv5_256_SEED_BYTES, hqcv5_256_PARAM_SECURITY_BYTES);

    // Parse ciphertext c_kem
    hqcv5_256_hqc_c_kem_from_string(&c_kem_t.c_pke, c_kem_t.salt, c_kem);
    ...
    hqcv5_256_hqc_pke_encrypt(&c_kem_prime_t.c_pke, ek_pke, (uint64_t *)m_prime, theta_prime);
    ...
    result -= 1;
    for (size_t i = 0; i < hqcv5_256_SHARED_SECRET_BYTES; ++i) {
        K_prime[i] = (K_prime[i] & result) ^ (K_bar[i] & ~result);
    }

#ifdef VERBOSE
    printf("\n\nek_pke: ");
    for (int i = 0; i < hqcv5_256_PUBLIC_KEY_BYTES; ++i) printf("%02x", ek_pke[i]);
    printf("\n\ndk_pke: ");
    for (int i = 0; i < hqcv5_256_SEED_BYTES; ++i) printf("%02x", dk_pke[i]);
    printf("\n\nc_kem: ");
    for (int i = 0; i < hqcv5_256_CIPHERTEXT_BYTES; ++i) printf("%02x", c_kem[i]);
    printf("\n\nm_prime: ");
    hqcv5_256_vect_print((uint64_t *)m_prime, hqcv5_256_PARAM_SECURITY_BYTES);
    printf("\n\nH(ek_kem): ");
    for (int i = 0; i < hqcv5_256_SEED_BYTES; ++i) printf("%02x", hash_ek_kem[i]);
    printf("\n\ntheta_prime: ");
    for (int i = 0; i < hqcv5_256_SEED_BYTES; ++i) printf("%02x", theta_prime[i]);
    printf("\n\n\n# Checking Ciphertext - Begin #");
    printf("\n\nc_kem_prime_t.c_pke.u: ");
    hqcv5_256_vect_print(c_kem_prime_t.c_pke.u, hqcv5_256_VEC_N_SIZE_BYTES);
    printf("\n\nc_kem_prime_t.c_pke.v: ");
    hqcv5_256_vect_print(c_kem_prime_t.c_pke.v, hqcv5_256_VEC_N1N2_SIZE_BYTES);
    printf("\n\nsalt: ");
    for (int i = 0; i < hqcv5_256_SALT_BYTES; ++i) printf("%02x", c_kem_prime_t.salt[i]);
    printf("\n\n# Checking Ciphertext - End #\n");
    printf("\n\nK_prime: ");
    for (int i = 0; i < hqcv5_256_SHARED_SECRET_BYTES; ++i) printf("%02x", K_prime[i]);
#endif

    // Zeroize sensitive data
    hqcv5_256_memset_zero(dk_pke, sizeof dk_pke);
    hqcv5_256_memset_zero(sigma, sizeof sigma);
    hqcv5_256_memset_zero(m_prime, sizeof m_prime);
    hqcv5_256_memset_zero(K_theta_prime, sizeof K_theta_prime);
    hqcv5_256_memset_zero(K_bar, sizeof K_bar);
    hqcv5_256_memset_zero(theta_prime, sizeof theta_prime);
