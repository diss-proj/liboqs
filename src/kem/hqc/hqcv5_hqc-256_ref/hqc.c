...
void hqcv5_256_hqc_pke_keygen(uint8_t *ek_pke, uint8_t *dk_pke, uint8_t *seed) {
    ...
    memcpy(ek_pke, seed_ek, hqcv5_256_SEED_BYTES);
    memcpy(ek_pke + hqcv5_256_SEED_BYTES, s, hqcv5_256_VEC_N_SIZE_BYTES);

    // Parse decryption key to string
    memcpy(dk_pke, seed_dk, hqcv5_256_SEED_BYTES);

#ifdef VERBOSE
    printf("\n\nseed_dk: ");
    for (int i = 0; i < hqcv5_256_SEED_BYTES; ++i) printf("%02x", seed_dk[i]);
    printf("\n\nseed_ek: ");
    for (int i = 0; i < hqcv5_256_SEED_BYTES; ++i) printf("%02x", seed_ek[i]);
    printf("\n\ny: ");
    hqcv5_256_vect_print(y, hqcv5_256_VEC_N_SIZE_BYTES);
    printf("\n\nx: ");
    hqcv5_256_vect_print(x, hqcv5_256_VEC_N_SIZE_BYTES);
    printf("\n\nh: ");
    hqcv5_256_vect_print(h, hqcv5_256_VEC_N_SIZE_BYTES);
    printf("\n\ns: ");
    hqcv5_256_vect_print(s, hqcv5_256_VEC_N_SIZE_BYTES);
#endif

    // Zeroize sensitive data
    hqcv5_256_memset_zero(keypair_seed, sizeof keypair_seed);
    hqcv5_256_memset_zero(x, sizeof x);
    hqcv5_256_memset_zero(y, sizeof y);
...
void hqcv5_256_hqc_pke_encrypt(hqcv5_256_ciphertext_pke_t *c_pke, const uint8_t *ek_pke, const uint64_t *m, const uint8_t *theta) {
    ...
    uint64_t r2[hqcv5_256_VEC_N_SIZE_64] = {0};
    uint64_t e[hqcv5_256_VEC_N_SIZE_64] = {0};
    ...
    hqcv5_256_vect_sample_fixed_weight2(&theta_xof_ctx, r2, hqcv5_256_PARAM_OMEGA_R);
    hqcv5_256_vect_sample_fixed_weight2(&theta_xof_ctx, e, hqcv5_256_PARAM_OMEGA_E);
    hqcv5_256_vect_sample_fixed_weight2(&theta_xof_ctx, r1, hqcv5_256_PARAM_OMEGA_R);

    // Compute u = r1 + r2.h
    hqcv5_256_vect_mul(c_pke->u, r2, h);
    hqcv5_256_vect_add(c_pke->u, r1, c_pke->u, hqcv5_256_VEC_N_SIZE_64);

    // Compute v = C.encode(m)
    hqcv5_256_code_encode(c_pke->v, m);

    // Compute v = C.encode(m) + Truncate(s.r2 + e)
    hqcv5_256_vect_mul(tmp, r2, s);
    hqcv5_256_vect_add(tmp, e, tmp, hqcv5_256_VEC_N_SIZE_64);
    hqcv5_256_vect_truncate(tmp);
    hqcv5_256_vect_add(c_pke->v, c_pke->v, tmp, hqcv5_256_VEC_N1N2_SIZE_64);
    ...
    hqcv5_256_vect_print(s, hqcv5_256_VEC_N_SIZE_BYTES);
    printf("\n\nr1: ");
    hqcv5_256_vect_print(r1, hqcv5_256_VEC_N_SIZE_BYTES);
    printf("\n\nr2: ");
    hqcv5_256_vect_print(r2, hqcv5_256_VEC_N_SIZE_BYTES);
    printf("\n\ne: ");
    hqcv5_256_vect_print(e, hqcv5_256_VEC_N_SIZE_BYTES);
    printf("\n\nTruncate(s.r2 + e): ");
    hqcv5_256_vect_print(tmp, hqcv5_256_VEC_N1N2_SIZE_BYTES);
    printf("\n\nc_pke->u: ");
    hqcv5_256_vect_print(c_pke->u, hqcv5_256_VEC_N_SIZE_BYTES);
    printf("\n\nc_pke->v: ");
    hqcv5_256_vect_print(c_pke->v, hqcv5_256_VEC_N1N2_SIZE_BYTES);
#endif

    // Zeroize sensitive data
    hqcv5_256_memset_zero(r1, sizeof r1);
    hqcv5_256_memset_zero(r2, sizeof r2);
    hqcv5_256_memset_zero(e, sizeof e);
    hqcv5_256_memset_zero(tmp, sizeof tmp);
    hqcv5_256_memset_zero(&theta_xof_ctx, sizeof theta_xof_ctx);
}
...
uint8_t hqcv5_256_hqc_pke_decrypt(uint64_t *m, const uint8_t *dk_pke, const hqcv5_256_ciphertext_pke_t *c_pke) {
    uint64_t y[hqcv5_256_VEC_N_SIZE_64] = {0};
    uint64_t tmp1[hqcv5_256_VEC_N_SIZE_64] = {0};
    uint64_t tmp2[hqcv5_256_VEC_N_SIZE_64] = {0};

    // Parse decryption key dk_pke
    hqcv5_256_hqc_dk_pke_from_string(y, dk_pke);

    // Compute u.y
    hqcv5_256_vect_mul(tmp1, y, c_pke->u);
    // Truncate(u.y)
    hqcv5_256_vect_truncate(tmp1);
    // Compute v - Truncate(u.y)
    hqcv5_256_vect_add(tmp2, c_pke->v, tmp1, hqcv5_256_VEC_N1N2_SIZE_64);
    ...
    hqcv5_256_vect_print(y, hqcv5_256_VEC_N_SIZE_BYTES);
    printf("\n\nTruncate(u.y): ");
    hqcv5_256_vect_print(tmp1, hqcv5_256_VEC_N1N2_SIZE_BYTES);
    printf("\n\nv - Truncate(u.y): ");
    hqcv5_256_vect_print(tmp2, hqcv5_256_VEC_N1N2_SIZE_BYTES);
#endif

    // Compute plaintext m
    hqcv5_256_code_decode(m, tmp2);

    // Zeroize sensitive data
    hqcv5_256_memset_zero(y, sizeof y);
    hqcv5_256_memset_zero(tmp1, sizeof tmp1);
    hqcv5_256_memset_zero(tmp2, sizeof tmp2);
