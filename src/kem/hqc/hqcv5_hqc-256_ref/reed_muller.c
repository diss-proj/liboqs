 ...
 *
 * Calculates the ceiling of hqcv5_256_PARAM_N2/128 to determine how many
 * copies of each 128-bit codeword are used in the code expansion.
 */
#define hqcv5_256_MULTIPLICITY CEIL_DIVIDE(hqcv5_256_PARAM_N2, 128)

/**
 * @typedef hqcv5_256_rm_expanded_cdw
 * @brief Internal representation of a codeword with each bit expanded to a 16-bit signed value.
 */
typedef int16_t hqcv5_256_rm_expanded_cdw[128];
...
void hqcv5_256_encode(hqcv5_256_rm_codeword_t *word, int32_t message);
void hqcv5_256_hadamard(hqcv5_256_rm_expanded_cdw *src, hqcv5_256_rm_expanded_cdw *dst);
void hqcv5_256_expand_and_sum(hqcv5_256_rm_expanded_cdw *dest, hqcv5_256_rm_codeword_t src[]);
int32_t hqcv5_256_find_peaks(hqcv5_256_rm_expanded_cdw *transform);
...
void hqcv5_256_hadamard(hqcv5_256_rm_expanded_cdw *src, hqcv5_256_rm_expanded_cdw *dst) {
    ...
    hqcv5_256_rm_expanded_cdw *p1 = src;
    hqcv5_256_rm_expanded_cdw *p2 = dst;
    for (int32_t pass = 0; pass < 7; pass++) {
        ...
        hqcv5_256_rm_expanded_cdw *p3 = p1;
        p1 = p2;
        p2 = p3;
    }
}
...
void hqcv5_256_expand_and_sum(hqcv5_256_rm_expanded_cdw *dest, hqcv5_256_rm_codeword_t src[]) {
    ...
    for (int32_t copy = 1; copy < hqcv5_256_MULTIPLICITY; copy++) {
        for (int32_t part = 0; part < 4; part++) {
            for (int32_t bit = 0; bit < 32; bit++) {
 ...
 *
 * The message consists of N1 bytes each byte is encoded into hqcv5_256_PARAM_N2 bits,
 * or hqcv5_256_MULTIPLICITY repeats of 128 bits
 *
 * @param[out] cdw Array of size hqcv5_256_VEC_N1N2_SIZE_64 receiving the encoded message
 * @param[in] msg Array of size hqcv5_256_VEC_N1_SIZE_64 storing the message
 */
void hqcv5_256_reed_muller_encode(uint64_t *cdw, const uint64_t *msg) {
    uint8_t *message_array = (uint8_t *)msg;
    hqcv5_256_rm_codeword_t *codeArray = (hqcv5_256_rm_codeword_t *)cdw;
    for (size_t i = 0; i < hqcv5_256_VEC_N1_SIZE_BYTES; i++) {
        // fill entries i * hqcv5_256_MULTIPLICITY to (i+1) * hqcv5_256_MULTIPLICITY
        int32_t pos = i * hqcv5_256_MULTIPLICITY;
        // encode first word
        hqcv5_256_encode(&codeArray[pos], message_array[i]);
        // copy to other identical codewords
        for (size_t copy = 1; copy < hqcv5_256_MULTIPLICITY; copy++) {
            memcpy(&codeArray[pos + copy], &codeArray[pos], sizeof(hqcv5_256_rm_codeword_t));
        }
    }
}
 ...
 *
 * @param[out] msg Array of size hqcv5_256_VEC_N1_SIZE_64 receiving the decoded message
 * @param[in] cdw Array of size hqcv5_256_VEC_N1N2_SIZE_64 storing the received word
 */
void hqcv5_256_reed_muller_decode(uint64_t *msg, const uint64_t *cdw) {
    uint8_t *message_array = (uint8_t *)msg;
    hqcv5_256_rm_codeword_t *codeArray = (hqcv5_256_rm_codeword_t *)cdw;
    hqcv5_256_rm_expanded_cdw expanded;
    for (size_t i = 0; i < hqcv5_256_VEC_N1_SIZE_BYTES; i++) {
        // collect the codewords
        hqcv5_256_expand_and_sum(&expanded, &codeArray[i * hqcv5_256_MULTIPLICITY]);
        // apply hadamard transform
        hqcv5_256_rm_expanded_cdw transform;
        hqcv5_256_hadamard(&expanded, &transform);
        // fix the first entry to get the half Hadamard transform
        transform[0] -= 64 * hqcv5_256_MULTIPLICITY;
        // finish the decoding
        message_array[i] = hqcv5_256_find_peaks(&transform);
    }
}
